...
class ConversationAgent:
    ...
    def __init__(self, query_api, name, background, relationship_goals, other_major_goals, personal_context) -> None:
        self.query_api = query_api # Accepts 
        self.name = name
    ...
    def get_current_status(self):
        ...
        if self.wants_relationship_to_end:
            prompt += f""" They are also considering ending the relationship."""
    ...
    def reflect_on_conversation(self, conversation_block):
        ...
        prompt =  self.get_current_status() + textwrap.dedent(f"""\
                ...
                First, print "Feelings:" with a few words, e.g. 'curious and attracted to their new date' or 'angry and resentful for being lied to'
...
class RelationshipConversationContext():
    ...
    def get_general_prompt(self):
        assignment = textwrap.dedent(f"""\
                        ...
                        If both people need to end the relationship, print "END OF NARRATIVE" on a new line.
                        """).replace("\n", " ")
    ...
    def check_conversation_ended(self, snippets):
        if "END OF CONVERSATION" in snippets or "END OF NARRATIVE" in snippets or self.rounds_in_conversation >= self.max_rounds_in_conversation:
            self.total_conversations += 1
            self.continue_conversation = False

    def check_narrative_ended(self, snippets):
        if "END OF NARRATIVE" in snippets:
